...
if len(sys.argv) >1:
	...
	for arg in sys.argv[1:]:
		if re.match('\d{9}', arg):
			...
			if os.path.splitext(arg)[1] in ['.xls','.xlsx']:
				edis += xlspget(arg,'\d{9}')
			elif os.path.splitext(arg)[1] in ['.docx','doc']:
				edis += docpget(arg, '\d{9}')
			else:
				try:
					with open(arg,'r') as fp:
						data = fp.read()
						edis += re.findall('\d{9}', data)
				except:
					pass
...
# maxlenth = max(map(lambda x:len(repr(x)),ret))*2
# print(maxlenth)
# pp = pprint.PrettyPrinter(width=maxlenth)
# pp.pprint(ret)
